...
function addPost(req, res) {
    ...
    if (req.file) {
        ...
        Image.create({img: img}, (err, image) => {
            if (!err) {
                Post.create({title: reqPost.title, content: reqPost.content, user: reqPost.user, image: image._id}, (err, post) => {
                    if (!err) {
...
function getPosts(req, res) {
    Post.find({}).sort('-updatedAt').find().populate([{path: 'user', select: ['firstName', 'lastName']}, {path: 'image', select: 'img'}]).then(posts => {
        if (!posts.length) {
